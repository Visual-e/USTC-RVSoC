...
Рисунок выше показывает структуру SoC. Арбитр шины bus_router является центральным элементом SoC, который имеет три ведущих интерфейса и пять подчиненных интерфейсов. Шина, используемая этим SoC, не относится ни к какому либо стандарту (например, к шине AXI или APB), это простая авторская шина, которая называется naive_bus.
